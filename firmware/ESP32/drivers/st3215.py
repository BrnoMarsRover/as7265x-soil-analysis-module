# drivers/st3215.py
# Waveshare ST3215 serial bus servo backend, on ESP32 UART2 through a
# Waveshare Serial Bus Servo Driver Board.
#
# The ST3215 is a smart actuator: a 360 degree magnetic encoder with 4096
# counts per revolution, its own closed position loop, and a status packet
# on every command carrying position, speed, load, voltage, temperature,
# current and a moving flag. Nothing here is timed, and nothing is
# assumed - a movement is finished when the servo SAYS it has stopped and
# the encoder agrees.
#
# POWER: this module drives no servo power of any kind. The ESP32 PCB
# provides only UART TX, UART RX and a common ground reference to the
# driver board; the servo supply is external and outside the control of
# this firmware. There is deliberately no power-enable concept here.
#
# Layers, bottom to top:
#
#     UART transport          _ensure_uart, _flush_input, _read_into
#     frame scan / validate   _scan_frame, _receive
#     transaction + retry     _transaction
#     register access         read/write byte/word/register
#     servo semantics         position, mode, torque, feedback
#     verified movement       move_relative, move_slots, half_turn
#
# The protocol constants and the pure encodings live in
# st3215_registers.py, which cites every official source and records where
# the supplied reference driver disagrees with it.
#
# This module knows about a servo. It knows nothing about a carousel,
# slots, samples or geometry.
...
from drivers.servo_base import (
    CCW,
    CW,
    ServoError,
    ServoNotSupportedError,
    capabilities,
    direction_sign,
    validate_direction,
)

# Re-exported for callers that want the protocol vocabulary without
# reaching into the registers module. These are names, not new values.
MODE_POSITION = regs.MODE_POSITION
MODE_WHEEL = regs.MODE_WHEEL
MODE_PWM = regs.MODE_PWM
MODE_STEP = regs.MODE_STEP
MODE_NAMES = regs.MODE_NAMES
...
class ST3215:
    ...
    def capabilities(self):
        return capabilities(
            ...
            encoder=True,
            timed_positioning=False,
            telemetry=True,
            torque_control=True,
            verified_movement=True,
    ...
    def counts_to_degrees(self, counts):
        return regs.counts_to_degrees(counts, self.counts_per_rev)

    def degrees_to_counts(self, degrees):
        return regs.degrees_to_counts(degrees, self.counts_per_rev)
    ...
    def _scan_frame(self, buffer, sent, expected_length, seen):
        ...
        while index + 4 <= length_of_buffer:
            if (
                buffer[index] != regs.HEADER_BYTE
                or buffer[index + 1] != regs.HEADER_BYTE
            ):
                index += 1
            ...
            if length < 2 or length > regs.MAX_PACKET_BODY:
                seen["garbage"] += 1
                index += 1
            ...
            self.last_status_flags = regs.decode_status_flags(frame[4])
    ...
    def _receive(self, sent, expected_length, deadline):
        ...
        while True:
            ...
            if len(buffer) > regs.MAX_PACKET_BODY * 4:
    ...
    def _transaction(self, instruction, params, expected_length,
        ...
        """
        packet = regs.build_packet(self.servo_id, instruction, params)
    ...
    def ping(self):
        ...
        """
        try:
            self._transaction(regs.INST_PING, b"", 0)
    ...
    def read_word(self, address):
        ...
        return regs.bytes_word(data[0], data[1])

    def write_byte(self, address, value):
        return self.write_register(address, bytes([int(value) & 0xFF]))

    def write_word(self, address, value):
        return self.write_register(address, regs.word_bytes(value))
    ...
    def read_position(self):
        ...
        """
        return regs.decode_signed(self.read_word(regs.REG_PRESENT_POSITION))

    def read_mode(self, refresh=True):
        """Operating mode, read from the servo rather than assumed."""
        if refresh or self.mode is None:
            self.mode = self.read_byte(regs.REG_MODE)

        return self.mode

    def read_moving(self):
        return bool(self.read_byte(regs.REG_MOVING))

    def read_torque_enabled(self):
        return bool(self.read_byte(regs.REG_TORQUE_SWITCH))
    ...
    def read_feedback(self):
        ...
        """
        data = self.read_register(regs.FEEDBACK_FIRST, regs.FEEDBACK_LENGTH)

        def word(register):
            index = register - regs.FEEDBACK_FIRST

            return regs.bytes_word(data[index], data[index + 1])

        def byte(register):
            return data[register - regs.FEEDBACK_FIRST]

        position = regs.decode_signed(word(regs.REG_PRESENT_POSITION))
        status_byte = byte(regs.REG_SERVO_STATUS)

        self.last_status = status_byte
        self.last_status_flags = regs.decode_status_flags(status_byte)
        self.moving = bool(byte(regs.REG_MOVING))
        ...
        return {
            ...
            "speed_steps_per_s": regs.decode_signed(
                word(regs.REG_PRESENT_SPEED)
            ),
            "load_permille": regs.decode_signed(
                word(regs.REG_PRESENT_LOAD), 10
            ),
            "voltage_v": round(
                byte(regs.REG_PRESENT_VOLTAGE) * regs.VOLTAGE_V_PER_LSB, 1
            ),
            "temperature_c": byte(regs.REG_PRESENT_TEMPERATURE),
            "current_ma": round(
                regs.decode_signed(word(regs.REG_PRESENT_CURRENT))
                * regs.CURRENT_MA_PER_LSB, 1
            ),
            "moving": self.moving,
            "status": status_byte,
            "status_flags": list(self.last_status_flags),
    ...
    def enable_torque(self):
        ...
        self.write_byte(regs.REG_TORQUE_SWITCH, regs.TORQUE_ON)
    ...
    def disable_torque(self):
        ...
        """
        self.write_byte(regs.REG_TORQUE_SWITCH, regs.TORQUE_OFF)
    ...
    def configure_mode(self, mode=None):
        ...
        self.write_byte(regs.REG_LOCK, regs.LOCK_OPEN)

        try:
            self.write_byte(regs.REG_MODE, mode)
            self.write_word(regs.REG_MIN_ANGLE_LIMIT, 0)
            self.write_word(regs.REG_MAX_ANGLE_LIMIT, 0)

        finally:
            self.write_byte(regs.REG_LOCK, regs.LOCK_CLOSED)
        ...
        return {
            ...
            "min_angle_limit": self.read_word(regs.REG_MIN_ANGLE_LIMIT),
            "max_angle_limit": self.read_word(regs.REG_MAX_ANGLE_LIMIT),
    ...
    def write_goal(self, value, speed=None, acceleration=None):
        ...
        payload = (
            bytes([acceleration])
            + regs.word_bytes(regs.encode_signed(value))
            + regs.word_bytes(0)                # goal time, PWM mode only
            + regs.word_bytes(speed)
        )
        ...
        # twice.
        self.write_register(regs.REG_ACCELERATION, payload, retries=0)
    ...
    def move_relative(self, counts, speed=None, acceleration=None):
        ...
        minimum = self.read_word(regs.REG_MIN_ANGLE_LIMIT)
        maximum = self.read_word(regs.REG_MAX_ANGLE_LIMIT)
    ...
    def _execute_move(self, goal, start, speed, acceleration,
        ...
        expected = regs.wrap_counts(start + requested, self.counts_per_rev)
        ...
        position = self.read_position()
        error = regs.centred_error(expected - position, self.counts_per_rev)
    ...
    def diagnostics(self):
        ...
        try:
            ...
            step("ping", self.ping)
            step("id", lambda: self.read_byte(regs.REG_ID))

            baud_code = step(
                "baud_code", lambda: self.read_byte(regs.REG_BAUD_RATE)
            )
            mode = step("mode", lambda: self.read_mode())
            torque = step("torque", self.read_torque_enabled)
            step("angle_limits", lambda: {
                "min": self.read_word(regs.REG_MIN_ANGLE_LIMIT),
                "max": self.read_word(regs.REG_MAX_ANGLE_LIMIT),
            })
            ...
            report["baud_reported"] = regs.BAUD_CODES.get(baud_code)
            report["baud_matches"] = (
                regs.BAUD_CODES.get(baud_code) == self.baudrate
            )
        ...
        report["bus"] = dict(self.stats)
        report["capabilities"] = self.capabilities()
...
def _describe_probe(buffer, sent, servo_id, expect_echo=True):
    ...
    while index + 4 <= len(buffer):
        ...
        if regs.checksum(frame[2:-1]) != frame[-1]:
        ...
        if found_id == servo_id:
            ...
            report["status_flags"] = regs.decode_status_flags(frame[4])
...
def _probe_id(uart, servo_id, timeout_ms, expect_echo=True):
    ...
    """
    packet = regs.build_packet(servo_id, regs.INST_PING, b"")
