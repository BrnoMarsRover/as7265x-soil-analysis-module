...
def run_check(plan=None, site_plan=None, yard=None, registry=None,
    ...
    try:
        from Measurements import evidence as evidence_module
        ...
        checks.append(Check(
            ...
            "Measurements did not import: {}".format(error),
            ["O/SCI-150"], blocks_run=True,
        ))

    try:
        from DecisionModel import engine as decision_engine
        ...
        checks.append(Check(
            "decision model", FAIL,
            "DecisionModel did not import: {}".format(error),
            ["O/SCI-150"],
        ))
...
def configuration_payload(registry=None, calibration_store=None,
    ...
    try:
        from Measurements import config as measurement_config
