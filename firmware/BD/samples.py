...
beside the science data it was produced against:
    ...
    firmware/BD/data/DB1.json     reference materials     READ ONLY
    firmware/BD/data/calibration_legacy.json   fixed White/Dark        READ ONLY
...
# All software lives under firmware/ (see Documentation/ARCHITECTURE.md), so this is firmware/,
# not the repository root. Named for what it actually is.
FIRMWARE_ROOT = BD_DIR.parent

ARCHIVE_PATH = config.SAMPLES_FILE

# Two retired layouts, migrated automatically on first load. Both are read
# only — the old files are left exactly where they are, so a migration
# that goes wrong costs nothing.
#
#   generation 1  firmware/PC/data/  index + one file per sample
#   generation 2  firmware/BD/samples.json  flat archive beside the DB
#
# Generation 2 is where the archive lived before ARCHITECTURE.md moved it into
# BD/samples/. An operator upgrading across that change must not silently
# lose the run's only irreplaceable data.
LEGACY_DATA_DIR = FIRMWARE_ROOT / "PC" / "data"
LEGACY_INDEX_PATH = LEGACY_DATA_DIR / "samples.json"
LEGACY_RECORD_DIR = LEGACY_DATA_DIR / "samples"

LEGACY_FLAT_ARCHIVE = BD_DIR / "samples.json"
...
class SampleStore:
    ...
    firmware/BD/samples.json sits beside DB1.json and
    calibration_legacy.json - the measured half of the science data, and the
    ...
    def _load_legacy(self):
        ...
        # Generation 2 first: a whole archive in the pre-ARCHITECTURE.md location
        # is adopted as-is rather than rebuilt record by record.
        flat = _read_json(LEGACY_FLAT_ARCHIVE)

        if isinstance(flat, dict) and isinstance(flat.get("samples"), list):
            self.migrated_from = str(LEGACY_FLAT_ARCHIVE)
            flat.setdefault("version", ARCHIVE_VERSION)
