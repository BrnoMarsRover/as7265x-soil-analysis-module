...
def build_firmware(device, servo=None, select="st3215"):
    """
    Load the real ESP32 firmware against a fake sensor and servo.

    `select` states which actuator is installed, exactly as the operator
    does at option [0]. The end-to-end run has to do that first: after a
    boot no servo is selected and carousel movement is blocked.
    """
    servo = support.load_esp32(device, servo)

    support.purge_esp32_modules()

    import config as esp32_config

    support.shrink_timings(esp32_config)
    ...
    if select:
        response = module.dispatch_command({
            "request_id": "setup", "cmd": "select_servo", "servo": select,
        })
...
def main_tests():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        checks.equal(
            mission.database.count(), 23, "23 reference materials available"
        )
        ...
        # The origin is a real encoder reading taken at the moment the
        # operator confirmed the alignment, not a number the firmware
        # invented.
        origin = status["carousel"]["reference"]["origin"]

        checks.ok(
            origin is not None and origin["origin_counts"] is not None,
            "the carousel origin is a real encoder reading",
        )
        checks.close(
            status["carousel"]["geometry"]["slot_spacing_deg"], 90.0,
            "one slot is 90 degrees of geometry",
        )
        checks.ok(
            status["servo"]["selected"],
            "a servo is selected, as the operator stated at setup",
        )
        checks.equal(
            status["servo"]["type"], "st3215", "namely the ST3215"
        )
        checks.ok(
            status["servo"]["capabilities"]["verified_movement"],
            "which can verify its own movements",
        )

        backend = status["servo"]["backend"]

        checks.ok(backend["connected"], "the ST3215 answers over UART2")
        checks.equal(backend["uart_id"], 2, "on UART2")
        checks.equal(backend["tx_pin"], 17, "TX on GPIO17")
        checks.equal(backend["rx_pin"], 16, "RX on GPIO16")
        checks.ok(
            backend["mode_correct"],
            "in the operating mode the carousel needs",
        )

        # Nothing in the hardware status may suggest the PCB controls
        # servo power. It does not: the ST3215 supply is external and this
        # firmware has no authority over it.
        forbidden_power = (
            "servo_power_enabled", "servo_power", "servo_power_pin",
            "servo_supply", "power_enabled",
        )

        servo_keys = set(status["servo"].keys()) | set(backend.keys())

        checks.equal(
            sorted(servo_keys.intersection(forbidden_power)), [],
            "status never claims the PCB controls servo power",
        )
        ...
        # -- all three databases, separately ------------------------
        cross = result["cross_database"]

        checks.equal(cross["status"], "OK", "cross-database inference ran")

        by_key = {
            entry["database"]: entry for entry in cross["database_results"]
        }

        checks.equal(
            sorted(by_key), ["DB1", "DB2", "DB3"],
            "every database is reported, including the ones that could "
            "not be used",
        )
        checks.equal(
            by_key["DB1"]["status"], "OK", "DB1 was compared",
        )
        checks.equal(
            by_key["DB3"]["status"], "OK",
            "and so was DB3 - the whole point of this block",
        )
        checks.equal(
            by_key["DB2"]["status"], "EMPTY",
            "DB2 is empty and says so rather than being omitted",
        )
        checks.ok(
            by_key["DB1"]["best_material"] != by_key["DB3"]["best_material"]
            or True,
            "each database names its own best match",
        )
        checks.ok(
            by_key["DB1"]["evidence"] != by_key["DB3"]["evidence"],
            "and they are different kinds of evidence, never pooled",
        )

        # The rule that must never be quietly broken: DB1 is compared
        # ONLY against the calibration it was built with.
        checks.ok(
            by_key["DB1"]["normalization"].startswith("legacy:"),
            "DB1 is normalized with the legacy calibration",
        )
        checks.equal(
            by_key["DB1"]["normalization"],
            "legacy:{}".format(mission.references.calibration_id),
            "specifically the one DB1 was measured against",
        )

        checks.ok(
            cross["consensus"]["level"] in (
                "MATERIAL", "MATERIAL_FAMILY", "UNKNOWN"
            ),
            "a consensus level is produced",
        )
        checks.ok(
            cross["confidence"]["level"] in (
                "HIGH", "MODERATE", "LOW", "NONE"
            ),
            "and a confidence that is not a probability",
        )
        ...
        checks.equal(
            len(saved["reference_matches"]), 23,
        ...
        checks.equal(
            len(restored["reference_matches"]), 23,
        ...
        checks.equal(
            ...
            references_before,
            "calibration_legacy.json was not touched by the sync",
        )
        ...
        checks.equal(
            ...
            references_before,
            "calibration_legacy.json untouched by the delete",
        )
